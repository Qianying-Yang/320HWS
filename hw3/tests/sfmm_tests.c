...
Test(sf_memsuite_student, testcase_1, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(1000);

	cr_assert_not_null(x);
	//cr_assert(get_heap_start() + PAGE_SZ == get_heap_end(), "Allocated wrong space!");
}

Test(sf_memsuite_student, testcase_2, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(sizeof(char));

	cr_assert_not_null(x);
	cr_assert(get_heap_start() + PAGE_SZ == get_heap_end(), "Allocated wrong space!");
}

Test(sf_memsuite_student, testcase_3, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(sizeof(int));
...
Test(sf_memsuite_student, testcase_4, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(sizeof(short));

	cr_assert_not_null(x);
	cr_assert(get_heap_start() + PAGE_SZ == get_heap_end(), "Allocated wrong space!");
}

Test(sf_memsuite_student, testcase_5, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(sizeof(int*));
